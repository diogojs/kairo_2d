...
class Player(Entity):
    def __init__(self, position: Optional[Vector2] = None):
        ...
        self._current_maps: Any = Game.get_all_by_type('LayerMap')
        tileset = Game.resources['girl-redhair-blueshirt-64px']

        # Components
        self.mover = Mover(speed=4, parent=self)
        ...
        self.collider = self.add_component(
            Collider(box=collider_box, parent=self, layers=[Layers.DEFAULT, Layers.FOREGROUND])
        )
    ...
    def update(self, *args, **kwargs) -> None:
        ...
        self.mover.update(position=self.position, keyboard_input=kb_input)
    ...
    def update_maps(self, current_maps: List['LayerMap']):
        self._current_maps = current_maps
        self.mover.current_maps = current_maps
